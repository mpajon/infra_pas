...
SOURCE_GITLAB_URL="https://gitlab.asnw.inetum.com/"
...
clone_project() {
  local repo_url=$1
  local repo_url_with_credentials=$(echo "$repo_url" | sed "s|https://|https://${encoded_username_mandalor}:${encoded_password_mandalor}@|")
...
create_project() {
  ...
  curl -s --insecure --noproxy '*' --request POST --header "PRIVATE-TOKEN: $gitlab_token" --header "Content-Type: application/json" \
...
destination_group=$(curl -s --insecure --noproxy '*' --header "PRIVATE-TOKEN: $DESTINATION_GITLAB_TOKEN" "$DESTINATION_GITLAB_URL/api/v4/groups/$(echo $DESTINATION_GROUP_PATH | sed 's/\//%2F/g')")
...
echo "$projects" | jq -c '.[]' | while read project; do
  ...
  destination_project=$(curl -s --insecure --noproxy '*' --header "PRIVATE-TOKEN: $DESTINATION_GITLAB_TOKEN" "$DESTINATION_GITLAB_URL/api/v4/projects/$(echo $DESTINATION_GROUP_PATH/$project_name | sed 's/\//%2F/g')")
  ...
  destination_repo_url=$(curl -s --insecure --noproxy '*' --header "PRIVATE-TOKEN: $DESTINATION_GITLAB_TOKEN" "$DESTINATION_GITLAB_URL/api/v4/projects/$(echo $DESTINATION_GROUP_PATH/$project_name | sed 's/\//%2F/g')" | jq -r '.http_url_to_repo')
